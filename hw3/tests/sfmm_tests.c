...
// ############################################
Test(sfmm_student_suite, malloc_an_ld, .timeout = TEST_TIMEOUT)
{
	sf_errno = 0;
	int *x = sf_malloc(sizeof(long double));

	cr_assert_not_null(x, "x is NULL!");

	*x = 16;

	cr_assert(*x == 16, "sf_malloc failed to give proper space for an long double!");
...
Test(sfmm_student_suite, malloc_an_int_array, .timeout = TEST_TIMEOUT)
{
	sf_errno = 0;
	int *arr = sf_malloc(sizeof(int) * 10);
	cr_assert_not_null(arr, "x is NULL!");
...
Test(sfmm_student_suite, malloc_an_int_array_2, .timeout = TEST_TIMEOUT)
{
	sf_errno = 0;
	int *arr = sf_malloc(sizeof(int) * 100);
	cr_assert_not_null(arr, "x is NULL!");

	assert_free_block_count(0, 1);
	assert_free_block_count(3632, 1);
	assert_free_list_size(9, 1);
...
Test(sfmm_student_suite, realloc_and_check, .timeout = TEST_TIMEOUT)
{
	sf_errno = 0;
	/* void *p = */ sf_malloc(16);
	void *q = sf_malloc(150);
	/* void *r = */ sf_malloc(5);
	sf_free(q);

	assert_free_block_count(0, 2);
	assert_free_block_count(160, 0);
	assert_free_block_count(3808, 1);
	cr_assert(sf_errno == 0, "sf_errno is not zero!");
}
Test(sfmm_basecode_suite, malloc_ten_pages, .timeout = TEST_TIMEOUT)
{
	sf_errno = 0;
	// We want to allocate up to exactly ten pages.
	void *x = sf_malloc(40960 - 48 - (sizeof(sf_header) + sizeof(sf_footer)));
