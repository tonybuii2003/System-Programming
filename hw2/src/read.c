    ...
    Score *s;
{
        ...
        s->flag = INVALID;
        if (checktoken("USERAW"))
                s->subst = USERAW;
        else if (checktoken("USENORM"))
                s->subst = USENORM;
        else if (checktoken("USELIKEAVG"))
                s->subst = USELIKEAVG;
        else if (checktoken("USECLASSAVG"))
                s->subst = USECLASSAVG;
        else
                s->flag = VALID;
...
Surname readsurname()
{
        ...
        if (!istoken())
                advancetoken();
        if (istoken())
                s = newstring(tokenptr, tokensize());
        else
        {
...
Id readid()
{
        ...
        if (!istoken())
                advancetoken();
        if (istoken())
                i = newstring(tokenptr, tokensize());
        else
        {
...
Atype readatype()
{
        ...
        if (!istoken())
                advancetoken();
        if (istoken())
                a = newstring(tokenptr, tokensize());
        else
        {
