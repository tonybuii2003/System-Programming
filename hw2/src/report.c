...
void reportfreqs(fd, s) // add void
    FILE *fd;
Stats *s;
{
  Classstats *csp;
...
void reportquantilesummaries(fd, s) // add void
    FILE *fd;
Stats *s;
{
  Classstats *csp;
  ...
  for (csp = s->cstats; csp != NULL; csp = csp->next)
  {
    ...
    min = scores[0];
    max = scores[n - 1];
    for (i = 0; i < (int)(sizeof(quantiles) / sizeof(*quantiles)); i++) // casted to int
    {
      fprintf(fd, "   ");
  ...
  for (csp = s->cstats; csp != NULL; csp = csp->next)
  {
    ...
    for (fp = csp->freqs; fp != NULL; fp = fp->next)
    {
      ...
      else
      {
        fprintf(fd, "(%6.2f,%3.2f)", fp->score,
                (float)fp->numless * 100.0 / csp->tallied); // change ***.** to %3.2f
      }
    }
    fprintf(fd, "\n");
    if (csp->sstats != NULL && csp->sstats->next == NULL)
    ...
    for (ssp = csp->sstats; ssp != NULL; ssp = ssp->next)
    {
      ...
      for (fp = ssp->freqs; fp != NULL; fp = fp->next)
      {
        ...
        {
          fprintf(fd, "(%6.2f,%3.2f)", fp->score,
                  (float)fp->numless * 100.0 / ssp->tallied); // change ***.** to %3.2f
        }
      }
      fprintf(fd, "\n");
    }
...
void reportmoments(fd, s) // add void
    FILE *fd;
Stats *s;
{
  Classstats *csp;
...
void reportscores(fd, c, nm) // void add, before there was nothing
    FILE *fd;
Course *c;
int *nm; // declare nm as int
{
  // Assignment *ap;
  Student *stp;
  Score *rscp, *nscp;
  ...
  for (stp = c->roster; stp != NULL; stp = stp->cnext)
  {
    ...
    for (rscp = stp->rawscores; rscp != NULL; rscp = rscp->next)
    {
      ...
      if (rscp->flag == INVALID)
        fprintf(fd, "  %-s", rscp->code);
      fprintf(fd, "\n");
      nscp = nscp->next;
    }
    fprintf(fd, "\n");
...
void reportcomposites(fd, c, nm) // make it void from nothing
    FILE *fd;
Course *c;
int *nm; // declare nm
{
...
void reporthistos(fd, c, s) // add void, nothing before
    FILE *fd;
Course *c;
Stats *s;
{
  ...
  for (stp = c->roster; stp != NULL; stp = stp->cnext)
  {
    if (stp->composite < min)
      min = stp->composite;
    if (stp->composite > max)
      max = stp->composite;
    cnt++;
  }
  ...
  for (csp = s->cstats; csp != NULL; csp = csp->next)
  {
     ...
     * Print histogram
     */
    histo(fd, bins, 0.0, max, csp->tallied); // change from histo to print_histo
  }
}

void histo(fd, bins, min, max, cnt) // add void // change from histo to print_histo
    FILE *fd;
int bins[], cnt;
float min, max;
{
  int row, col, cmax; // deleted pct
  /*
...
void reporttabs(FILE *fd, Course *c) // deleted int nm
{
  Assignment *ap;
  Student *stp;
  Score *rscp; // deleted *nscp
